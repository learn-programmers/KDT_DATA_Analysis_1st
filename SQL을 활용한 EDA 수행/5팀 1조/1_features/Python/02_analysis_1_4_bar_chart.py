...
# << 파일 데이터 읽어오기 >>
file = filepath + filename_tempo_low
print(file)
...
cnt = cnt_keyword(df['spotify_genre']).value_counts()
# print(cnt)
df_cnt = cnt.to_frame()
...
plt.bar(df_cnt['value'][:10], df_cnt['count'].values[:10], width=0.5)
plt.ylabel(df.columns[1], size=15, labelpad=15)
# plt.ylabel('key', size=15, labelpad=15)
plt.title("Top genres", size=18, pad=10)
plt.show()
